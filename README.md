...
`ms push --all` also pushes to **origin (GitHub)**. Forks / repos you don't own
...
| `ms push [--all] [--name N]` | Push `HEAD` to aliyun; `--all` (or policy `both`) also pushes to origin. `skip` repos are skipped. |
...
ms push --all               # also push to GitHub
